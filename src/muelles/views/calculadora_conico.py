...
def _calcular_muelle_conico(request):
    """Runs the conical calculator computation from the POST data.
    ...
    paso = float(datos_entrada_muelle['pitch'])
    ...
    paso_qty = paso * ureg.mm

    def func_diametro_medio(h):
    ...
    muelle.set_geometry(func_D=func_diametro_medio, func_p= lambda x: paso_qty, free_length=free_length_qty)
    ...
    muelle.empty_tables()
    muelle.add_load_position(longitud_inicial * ureg.mm)
    muelle.add_load_position(longitud_final * ureg.mm)
    ...
    pitch_actual = paso_qty
    # nr_coils = free_length_qty / paso_qty
...
@csrf_protect
@ensure_csrf_cookie
def calculadora_conico(request):
    """Conical compression spring calculator view"""
    resultado = None
...
@csrf_protect
def calculadora_conico_pdf(request):
    """Generates the PDF report for the conical spring calculator (opens inline)."""
    if request.method != 'POST':
        return HttpResponse(_('Usa el formulario de cónico para generar el PDF.'), status=405)
    try:
        _muelle, resultado, _li, _lf = _calcular_muelle_conico(request)
    except Exception as e:
        resultado = {'error': _('Error en los cálculos: %(error)s') % {'error': str(e)}}
    return build_spring_report_pdf_response(
        _('Reporte de Muelle Cónico'), resultado, 'muelle_conico_report.pdf'
    )


@csrf_protect
def calculadora_conico_animacion(request):
    """Generates a GIF animation of the conical spring compression (opens inline)."""
    if request.method != 'POST':
        return HttpResponse(_('Usa el formulario de cónico para generar la animación.'), status=405)
    try:
        muelle, _resultado, longitud_inicial, longitud_final = _calcular_muelle_conico(request)
    except Exception as e:
        return HttpResponse(
    ...
    return animation_http_response(gif_bytes, 'muelle_conico_animacion.gif')
